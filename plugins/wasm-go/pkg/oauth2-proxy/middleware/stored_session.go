	...
	return http.HandlerFunc(func(rw http.ResponseWriter, req *http.Request) {
		...
		proxyCallBack := func(args ...interface{}) {
			...
			updateKeysCallback := func(args ...interface{}) {
				...
				if session == nil {
					util.Logger.Infof("DEBUG: CALLBACK SESSION NIL")
				} else {
					util.Logger.Infof("DEBUG: CALLBACK SESSION :%+v", session)
				}
				scope.Session = session
				next.ServeHTTP(rw, req)
				if resumeFlag {
					if rw.Header().Get(util.ResponseCode) == strconv.Itoa(http.StatusOK) {
		...
		if session == nil {
			util.Logger.Infof("DEBUG: SESSION NIL")
		} else {
			util.Logger.Infof("DEBUG: SESSION :%+v", session)
		}

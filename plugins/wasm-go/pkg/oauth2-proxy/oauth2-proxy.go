...
import (
	...
	hackMiddleware "github.com/alibaba/higress/plugins/wasm-go/pkg/oauth2-proxy/middleware"
	"github.com/alibaba/higress/plugins/wasm-go/pkg/oauth2-proxy/providers"
	hackSession "github.com/alibaba/higress/plugins/wasm-go/pkg/oauth2-proxy/sessions"
...
func NewOAuthProxy(opts *options.Options) (*OAuthProxy, error) {
	sessionStore, err := hackSession.NewSessionStore(&opts.Session, &opts.Cookie)
	if err != nil {
...
func buildPreAuthChain(opts *options.Options) (alice.Chain, error) {
	chain := alice.New(hackMiddleware.NewScope(opts.ReverseProxy, "X-Request-Id"))
	return chain, nil
}
...
func buildSessionChain(opts *options.Options, provider providers.Provider, sessionStore sessionsapi.SessionStore, serviceClient wrapper.HttpClient) alice.Chain {
	...
	ss, loadSession := hackMiddleware.NewStoredSessionLoader(&hackMiddleware.StoredSessionLoaderOptions{
		SessionStore:          sessionStore,
...
func (p *OAuthProxy) SignOut(rw http.ResponseWriter, req *http.Request) {
	...
	if session != nil {
		...
		util.Logger.Info("Authenticated session found, add id_token_hint")
	}
	...
	if len(values) > 0 {
		...
		util.Logger.Infof("Redirect URL parsed: %+v", redirectURL)
		...
		util.Logger.Infof("Updated redirect URL with query parameters: %s", redirect)
	}
...
func (p *OAuthProxy) doOAuthStart(rw http.ResponseWriter, req *http.Request, overrides url.Values) {
	util.Logger.Info("Starting OAuth authentication process")
	extraParams := p.provider.Data().LoginURLParams(overrides)
	util.Logger.Infof("Extra params for login URL: %+v", extraParams)
	prepareNoCache(rw)
	...
	util.Logger.Infof("Log hack for provider data: %+v", p.provider.Data().StoredSession)
	...
	util.Logger.Infof("CSRF token created: %s", csrf)
	...
	util.Logger.Infof("Application redirect URL: %s", appRedirect)
	callbackRedirect := p.getOAuthRedirectURI(req)
	util.Logger.Infof("OAuth callback redirect URL: %s", callbackRedirect)

	loginURL := p.provider.GetLoginURL(
		callbackRedirect,
		encodeState(csrf.HashOAuthState(), appRedirect, p.encodeState),
		//base64.RawURLEncoding.EncodeToString([]byte(csrf.HashOAuthState())),
		//csrf.HashOAuthState(),
		csrf.HashOIDCNonce(),
		extraParams,
	)
	...
	util.Logger.Info("CSRF cookie set successfully")
...
func (p *OAuthProxy) OAuthCallback(rw http.ResponseWriter, req *http.Request) {
	...
	util.Logger.Info("CSRF cookie loaded successfully")
	callback := func(args ...interface{}) {
		session := args[0].(*sessionsapi.SessionState)
		csrf.ClearCookie(rw, req)
		util.Logger.Info("CSRF cookie cleared")
		...
		util.Logger.Infof("OAuth2 state decoded: nonce=%s, appRedirect=%s", nonce, appRedirect)
		...
		util.Logger.Info("CSRF token validated successfully")
		...
		updateKeysCallback := func(args ...interface{}) {
			...
			util.Logger.Info("Session validated successfully")
			if !p.redirectValidator.IsValidRedirect(appRedirect) {
				appRedirect = "/"
				util.Logger.Infof("Invalid redirect, defaulting to root: %s", appRedirect)
			}
			// set cookie, or deny
		...
		if p.provider.Data().NeedsVerifier {
			if _, err := (*p.provider.Data().Verifier.GetKeySet()).VerifySignature(req.Context(), session.IDToken); err != nil {
				util.Logger.Info("Updating keys due to signature verification failure")
				(*p.provider.Data().Verifier.GetKeySet()).UpdateKeys(p.client, p.provider.Data().VerifierTimeout, updateKeysCallback)
			} else {
				util.Logger.Info("Signature verification successful, proceeding without key update")
				updateKeysCallback()
			}
		} else {
			util.Logger.Info("No verifier needed, proceeding with updateKeysCallback")
			updateKeysCallback()
		}
	}
	util.Logger.Info("Starting OAuth redeem code process")
...
func (p *OAuthProxy) Proxy(rw http.ResponseWriter, req *http.Request) {
	util.Logger.Info("Starting proxy request")
	session, err := p.getAuthenticatedSession(rw, req)
	switch {
	case err == nil:
		...
		if p.passAuthorization {
			...
			util.Logger.Info("Authorization header add id token")
		}
	...
	case errors.Is(err, ErrAccessDenied):
		util.Logger.Info("Access denied due to authorization checks")
		if cookies, ok := rw.Header()[SetCookieHeader]; ok && len(cookies) > 0 {
			newCookieValue := strings.Join(cookies, ",")
...
func (p *OAuthProxy) getAuthenticatedSession(rw http.ResponseWriter, req *http.Request) (*sessionsapi.SessionState, error) {
	util.Logger.Info("Getting authenticated session")
	buf, _ := httputil.DumpRequest(req, true)
	util.Logger.Infof("DEBUG: session request body: %+v", string(buf))
	...
	if p.IsAllowedRequest(req) {
		util.Logger.Info("Request is allowed, returning session")
		return session, nil
	}

	if session == nil {
		util.Logger.Info("Session is nil, user needs to login")
	...
	util.Logger.Infof("Authorization result: %t", authorized)
	...
	util.Logger.Info("Session is valid and authorized, returning session")
...
func (p *OAuthProxy) SetVerifier(opts *options.Options) {
	if p.provider.Data().Verifier == nil && p.provider.Data().NeedsVerifier {
		...
		util.Logger.Infof("DEBUG: CodeChallangeMethod= %+v", p.provider.Data().CodeChallengeMethod)
		util.Logger.Infof("DEBUG: SupportedCodeChallangeMethods= %+v", p.provider.Data().SupportedCodeChallengeMethods)
	}
}
...
func redirectToLocation(rw http.ResponseWriter, location string) {
	util.Logger.Infof("Redirecting to location: %s", location)
	...
	for key, value := range rw.Header() {
		if strings.EqualFold(key, SetCookieHeader) {
			for _, value := range value {
				...
				util.Logger.Infof("Adding Set-Cookie header: %s", value)
			}
		} else {
			headersMap = append(headersMap, [2]string{key, strings.Join(value, ",")})
			util.Logger.Infof("Adding header: %s with value: %s", key, strings.Join(value, ","))
		}
	}
	util.Logger.Info("Sending HTTP Redirect response")
